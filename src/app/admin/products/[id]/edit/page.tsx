...
export default function EditProductPage() {
    ...
    fetch(`/api/admin/products/${id}`)
      ...
      .then((product) => {
        setForm({
          ...
          materialDetails: product.materialDetails, sizingDetails: product.sizingDetails,
          returnDetails: product.returnDetails, published: product.published,
        });
        setSizes(product.sizes.map((s: { label: string; stock: number }) => ({ label: s.label, stock: s.stock })));
        setImages(product.images.map((i: { url: string; alt: string }) => ({ url: i.url, alt: i.alt })));
        setLoading(false);
      });
  }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else { alert("Failed to update"); setSaving(false); }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input type="text" value={form.badge} onChange={(e) => setForm((f) => ({ ...f, badge: e.target.value }))} className={inputClass} />
          </div>
        </div>
            ...
            {sizes.map((size, i) => (
              <div key={i} className="flex gap-2 items-center">
                <input type="text" value={size.label} onChange={(e) => { const u = [...sizes]; u[i].label = e.target.value; setSizes(u); }}
                  className="bg-[#111] border-2 border-infld-grey-mid text-infld-white px-3 py-1.5 text-sm w-24 focus:border-infld-yellow focus:outline-none" />
                <input type="number" value={size.stock} onChange={(e) => { const u = [...sizes]; u[i].stock = parseInt(e.target.value) || 0; setSizes(u); }}
                  className="bg-[#111] border-2 border-infld-grey-mid text-infld-white px-3 py-1.5 text-sm w-20 focus:border-infld-yellow focus:outline-none" />
