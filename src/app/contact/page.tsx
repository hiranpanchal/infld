...
export default function ContactPage() {
  const [settings, setSettings] = useState<ContactSettings>({
    ...
    subheading: "Questions? Collabs? Noise? We're here.",
  });
  ...
  useEffect(() => {
    fetch("/api/admin/content?pageKey=contact")
      ...
      .then((data: { blockKey: string; content: string }[]) => {
        const s = { ...settings };
        data.forEach((b) => {
          if (b.blockKey === "heading" && b.content) s.heading = b.content;
          if (b.blockKey === "subheading" && b.content) s.subheading = b.content;
        });
        setSettings(s);
      })
      .catch(() => {});
